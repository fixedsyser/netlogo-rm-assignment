...
def plot_run_length_vs_parameter(df, save_path, varied_cols):
    varied_cols = clean_column_names(varied_cols)
    for parameter in varied_cols:
        
        if parameter not in df.columns:
            print(f"[ℹ] {parameter} not found or not varying, skipping run length analysis")
    
        print(f"[📈] Generating run length vs {parameter} analysis")
        
        # Group by number_of_trees and calculate average run_length
        avg_data = df.groupby(parameter)['run_length'].agg(['mean']).reset_index()
        
        # Create the plot
        plt.figure(figsize=(10, 6))
        plt.plot(avg_data[parameter], avg_data['mean'], 
                    marker='o', linewidth=2, markersize=6)
        
        plt.xlabel(parameter)
        plt.ylabel("Average Run Length (Steps)")
        plt.title(f"Average Run Length vs {parameter}")
        plt.grid(True, alpha=0.3)
        
        plt.savefig(os.path.join(save_path, f"run_length_vs_{parameter}.png"), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"[✔] Run length vs {parameter} saved")
